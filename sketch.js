...
function keyReleased() {
  if (keyCode === 32) {
    if (playerArrows.length) {
      ...
      playerArrows[playerArrows.length - 1].trajectory = [];
    }
  }
}
function showArrows(index, arrows) {
  arrows[index].display();
  for (var i = 0; i < arrows[index].trajectory.length; i++) {
    fill("white");
    ellipse(arrows[index].trajectory[i].x, arrows[index].trajectory[i].y, 5, 5);
  }
}
